    ...
    <router-link :to="'/Patent/fixPriceInfo/' + bridge_Msg">
        <el-card class="box-card" :body-style="{ padding: '0px' }">
          ...
          <div style="padding:15px;">
            ...
            <div style="font-weight:bolder;font-size:22px;width:120px;float:right;line-height:25px;color:rgb(230, 94, 64)" class="omit" >  ¥ {{price_Msg}}</div>
            </div>

            <div id="name" class="omit" style="width:150px;height:30px;float:left">{{name_Msg}}</div>
            <div style="float:right;text-align:right;font-size:14px;width:60px;float:right;line-height:15px;color:rgb(147, 139, 138)" class="omit" > {{status_Msg}}</div>
